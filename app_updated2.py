...
with st.sidebar.expander("🌍 Country & Product Filters", expanded=False):
    selected_country = st.multiselect("Select Country", options=data['Country'].unique(), default=data['Country'].unique())
    selected_product = st.multiselect("Select Product", options=data['ProductName'].unique(), default=data['ProductName'].unique())
...
with st.sidebar.expander("⚙️ Advanced Filters", expanded=False):
    price_range = st.slider("Price Range", float(data['Price'].min()), float(data['Price'].max()), (float(data['Price'].min()), float(data['Price'].max())))
    quantity_range = st.slider("Quantity Range", int(data['Quantity'].min()), int(data['Quantity'].max()), (int(data['Quantity'].min()), int(data['Quantity'].max())))
    filtered_data = filtered_data[(filtered_data['Price'] >= price_range[0]) & (filtered_data['Price'] <= price_range[1])]
    filtered_data = filtered_data[(filtered_data['Quantity'] >= quantity_range[0]) & (filtered_data['Quantity'] <= quantity_range[1])]
